...
import CardsRealtimeStream, {
  type CardBalanceUpdate,
} from "./CardsRealtimeStream";
...
export default function CardsPageClient({
  ...
  const handleBalanceUpdate = useCallback(
    ({ cardId, delta }: CardBalanceUpdate) => {
      setCards((prev) =>
        prev.map((c) =>
          c.id === cardId
            ? {
                ...
                balance: Math.max(
                  0,
                  parseFloat((c.balance + delta).toFixed(2)),
                ),
              }
            : c,
        ),
  ...
  return (
    <div className="p-8">
      ...
      <p className="text-sm text-neutral-500 mb-5">
        Manage your virtual and prepaid cards.
      </p>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span
                    className={`text-lg font-bold tabular-nums transition-colors duration-700 ${
                      flashingCardId === selectedCard.id
                        ? "text-positive"
                        : "text-neutral-900"
                    }`}
                  >
